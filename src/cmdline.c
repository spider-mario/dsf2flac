...
const char *cmdline_parser_samplerate_values[] = { "88200", "176400", "352800",
		0 }; /*< Possible values for samplerate. */
...
int cmdline_parser_dump(FILE *outfile, struct gengetopt_args_info *args_info) {
	...
	if (args_info->samplerate_given)
		write_into_file(outfile, "samplerate", args_info->samplerate_orig,
				cmdline_parser_samplerate_values);
	if (args_info->bits_given)
		write_into_file(outfile, "bits", args_info->bits_orig,
				cmdline_parser_bits_values);
...
int cmdline_parser2(int argc, char **argv,
		struct gengetopt_args_info *args_info, int override, int initialize,
		int check_required) {
	int result;
	struct cmdline_parser_params params;
	...
	if (possible_values
		...
		if (short_opt != '-')
			fprintf(stderr,
					...
					package_name, (found == -2) ? "ambiguous" : "invalid",
					value, long_opt, short_opt,
					(additional_error ? additional_error : ""));
		else
...
int cmdline_parser_internal(int argc, char **argv,
	...
	while (1) {
		...
		static struct option long_options[] = { { "help", 0, NULL, 'h' }, {
				"version", 0, NULL, 'V' }, { "samplerate", 1, NULL, 'r' }, {
				"bits", 1, NULL, 'b' }, { "nodither", 0, NULL, 'n' }, { "scale",
				1, NULL, 's' }, { "infile", 1, NULL, 'i' }, { "outfile", 1,
				NULL, 'o' }, { "dop", 0, NULL, 'd' }, { 0, 0, 0, 0 } };

		c = getopt_long(argc, argv, "hVr:b:ns:i:o:d", long_options,
				&option_index);
		...
		case 'r': /* Output sample rate.  */
			...
			if (update_arg((void *) &(args_info->samplerate_arg),
					&(args_info->samplerate_orig),
					&(args_info->samplerate_given),
					&(local_args_info.samplerate_given), optarg,
					cmdline_parser_samplerate_values, "88200", ARG_INT,
					check_ambiguity, override, 0, 0, "samplerate", 'r',
					additional_error))
				goto failure;
		...
		case 'b': /* Output bitdepth.  */
			...
			if (update_arg((void *) &(args_info->bits_arg),
					...
					&(local_args_info.bits_given), optarg,
					cmdline_parser_bits_values, "24", ARG_INT, check_ambiguity,
					override, 0, 0, "bits", 'b', additional_error))
				goto failure;

			break;
		case 'n': /* Don't add dither before quantization.  */

			if (update_arg((void *) &(args_info->nodither_flag), 0,
					&(args_info->nodither_given),
		...
		case 's': /* Scale adjustment. Raw DSD has a modulation depth of approximately 0.5 so with no scaling the PCM peak level is approximately -6dB below 0dBFs.  */
			...
			if (update_arg((void *) &(args_info->scale_arg),
					...
					check_ambiguity, override, 0, 0, "scale", 's',
					additional_error))
				goto failure;
		...
		case 'i': /* Input DSF or DFF file.  */
			...
			if (update_arg((void *) &(args_info->infile_arg),
					...
					check_ambiguity, override, 0, 0, "infile", 'i',
					additional_error))
				goto failure;
		...
		case 'o': /* Output FLAC file, if not specified the output file be the same as the input file with the extension changed.  */
			...
			if (update_arg((void *) &(args_info->outfile_arg),
					...
					check_ambiguity, override, 0, 0, "outfile", 'o',
					additional_error))
				goto failure;
		...
		case 'd': /* Encode DSD data directly into FLAC file without conversion to PCM using DoP format (DSD over PCM).  */
			...
			if (update_arg((void *) &(args_info->dop_flag), 0,
					&(args_info->dop_given), &(local_args_info.dop_given),
					optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0,
					"dop", 'd', additional_error))
				goto failure;
